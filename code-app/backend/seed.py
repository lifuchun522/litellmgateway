...
def _ensure_gateway_seed(conn):
    if db.query_one("SELECT COUNT(*) AS c FROM gateway_application", (), conn)["c"] > 0:
        _ensure_gateway_audit_seed(conn)
        return
    app_id = db.execute(
        "INSERT INTO gateway_application (code, name, owner, data_grade, daily_quota, monthly_budget) VALUES (?, ?, ?, ?, ?, ?)",
        ("research-copilot", "研发智能助手", "AI 平台组", "INTERNAL", 2000000, 3000), conn,
    )[0]
    db.execute(
        "INSERT INTO gateway_credential (app_id, key_prefix, secret_hash, expires_at) VALUES (?, ?, ?, datetime('now', '+180 day'))",
        (app_id, "llmgw-rd-01", "sha256:demo-redacted"), conn,
    )
    alias_id = db.execute(
        "INSERT INTO gateway_alias (code, name, capabilities, visibility) VALUES (?, ?, ?, ?)",
        ("smart-chat", "通用智能对话", json.dumps(["chat", "stream", "tools"]), "INTERNAL"), conn,
    )[0]
    openai_id = db.execute(
        "INSERT INTO gateway_provider (code, name, protocol, region) VALUES (?, ?, ?, ?)",
        ("openai", "OpenAI", "OPENAI", "global"), conn,
    )[0]
    azure_id = db.execute(
        "INSERT INTO gateway_provider (code, name, protocol, region) VALUES (?, ?, ?, ?)",
        ("azure-openai", "Azure OpenAI", "OPENAI", "eastasia"), conn,
    )[0]
    endpoints = [
        (openai_id, "openai-primary", "OpenAI 主端点", "gpt-4.1-mini", "https://api.openai.com/v1", "CONFIDENTIAL", 1, 70, 82, "HEALTHY", "vault://llmgw/openai", 0.0004, 0.0016),
        (azure_id, "azure-fallback", "Azure 灾备端点", "gpt-4.1-mini", "https://example.openai.azure.com", "CONFIDENTIAL", 2, 30, 115, "HEALTHY", "vault://llmgw/azure", 0.0005, 0.0018),
    ]
    for endpoint in endpoints:
        endpoint_id = db.execute(
            """INSERT INTO gateway_endpoint
            (provider_id, code, name, model, base_url, data_grade, priority, weight, latency_ms, status, secret_ref, input_price, output_price)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""", endpoint, conn,
        )[0]
        db.execute("INSERT INTO gateway_alias_endpoint (alias_id, endpoint_id) VALUES (?, ?)", (alias_id, endpoint_id), conn)
    policy_id = db.execute(
        """INSERT INTO gateway_policy
        (name, app_code, alias_code, strategy, status, submitted_by, security_approver, sre_approver, snapshot_version)
        VALUES (?, ?, ?, ?, 'PUBLISHED', ?, ?, ?, ?)""",
        ("研发助手默认策略", "research-copilot", "smart-chat", "LATENCY_FIRST", "产品负责人", "安全审批员", "SRE审批员", "snap-20260915-001"), conn,
    )[0]
    db.execute(
        "INSERT INTO gateway_snapshot (version, policy_id, compiled_config, ack_summary) VALUES (?, ?, ?, ?)",
        ("snap-20260915-001", policy_id, json.dumps({"maxAttempts": 3, "fallback": "before-first-byte"}), "cn-a: 2/2, cn-b: 2/2"), conn,
    )
    request_id = db.execute(
        """INSERT INTO gateway_request
        (trace_id, app_code, alias_code, snapshot_version, status, prompt_tokens, completion_tokens, cost, latency_ms)
        VALUES (?, ?, ?, ?, 'SUCCEEDED', 486, 172, 0.0004696, 986)""",
        ("tr-demo-20260915", "research-copilot", "smart-chat", "snap-20260915-001"), conn,
    )[0]
    db.execute(
        "INSERT INTO gateway_attempt (request_id, sequence, endpoint_code, status, latency_ms) VALUES (?, 1, ?, 'SUCCEEDED', 986)",
        (request_id, "openai-primary"), conn,
    )
    db.execute(
        "INSERT INTO gateway_reconciliation (provider_code, period, gateway_cost, provider_cost, difference_rate, status) VALUES (?, ?, ?, ?, ?, ?)",
        ("openai", "2026-09", 1284.26, 1302.11, 0.0139, "MATCHED"), conn,
    )
    _ensure_gateway_audit_seed(conn)


def _ensure_gateway_audit_seed(conn):
    existing = db.query_one(
        "SELECT COUNT(*) AS c FROM audit_logs WHERE action LIKE 'SNAPSHOT_%' OR action LIKE 'POLICY_%' OR action LIKE 'GATEWAY_%'",
        (),
        conn,
    )["c"]
    if existing:
    ...
    db.execute(
        ...
        (
            ...
            json.dumps(
                {
                    "objectType": "gateway_snapshot",
                    "objectId": "snap-20260915-001",
                    "ackSummary": "cn-a: 2/2, cn-b: 2/2",
                },
                ensure_ascii=False,
            ),
        ),
